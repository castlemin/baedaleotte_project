...
export const ReportThreatRank: React.FC<IProps> = ({
  ...
  return (
    <>
      <DescriptionSection style={{ opacity: (position - 400) / 80 }}>
        <ReportSubtitle>
          <ReportDesc style={{ opacity: (position - 440) / 80 }}>
            현재 당신의 행정구의 위험도 순위는 {rank}위 입니다.
          </ReportDesc>
          {Number(rank) < Number(25 / 2) ? (
            <>
              <ReportDesc style={{ opacity: (position - 460) / 80 }}>
            ...
            <>
              ...
              <ReportDesc style={{ opacity: (position - 570) / 80 }}>
