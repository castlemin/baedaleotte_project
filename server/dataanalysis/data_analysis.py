...
def 내지역확진자all(data, region):
    ...
    # Add figure title
    fig.update_layout(
        title_text=f"{region} 전체/추가 확진자({reg.index[0].strftime('%Y-%m-%d')} ~ {reg.index[-1].strftime('%Y-%m-%d')})",
        template="gridon",
        bargap=0.5,
    )
    # Set x-axis title
...
def 백신현황(data):
    ...
    fig.update_layout(
        title_text=f"1차 / 2차 / 추가 백신 접종률 ({vac['접종일'].iloc[0]} ~ {vac['접종일'].iloc[-1]})", template="gridon"
    )
    #     return fig.to_json()
    return fig.to_json()
